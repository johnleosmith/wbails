...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleNewsletterNotification = (id, node) => {
        try {
            ...
            logger.error({ err }, "Safely intercepted structural failure in native newsletter notification element");
        }
    };
    const handleMexNewsletterNotification = (id, node) => {
        try {
            ...
            if (operation === Types_1.MexOperations.UPDATE) {
                ...
                if (contentPath?.thread_metadata) {
                    ev.emit('newsletter-settings.update', { id, update: contentPath.thread_metadata.settings });
                }
            }
        } catch (err) {
            logger.error({ err }, "Safely intercepted structural failure in MEX newsletter notification payload");
        }
    };
        ...
        const enqueue = (type, node) => {
            ...
            const promise = async () => {
                while (nodes.length && ws.isOpen) {
                    ...
                    await nodeProcessor(node).catch(err => onUnexpectedError(err, `offline core handler failure: ${type}`));
                }
                isProcessing = false;
            };
